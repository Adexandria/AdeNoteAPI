...
{
    public class FetchAllTicketsRequestHandler : IRequestHandler<FetchAllTicketsRequest, ActionResult<PaginatedResponse<TicketsDTO>>>
    {
        public FetchAllTicketsRequestHandler(ITicketRepository _ticketRepository, IUserRepository _userRepository,
   ...
   , IConfiguration config)
        ...
        public async Task<ActionResult<PaginatedResponse<TicketsDTO>>> Handle(FetchAllTicketsRequest request, CancellationToken cancellationToken)
        {
            var tickets = ticketRepository.GetTickets(request.PageNumber, request.PageSize).ToList();
